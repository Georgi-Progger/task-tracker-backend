...
func NewService(repo repo.Repository, jwtSecret string, poducer kafka.Producer, accessTokenTTL time.Duration, logger logger.Logger) Service {
	return Service{
		AuthService:  NewAuthService(repo.UserReposetory, repo.RefreshTokenRepository, *NewEmailService(*NewTaskSrvice(repo.TaskRepository), poducer, logger), *NewUserService(repo.UserReposetory), jwtSecret, accessTokenTTL, logger),
		UserService:  NewUserService(repo.UserReposetory),
		TaskService:  NewTaskSrvice(repo.TaskRepository),
		EmailService: NewEmailService(*NewTaskSrvice(repo.TaskRepository), poducer, logger),
	}
}
