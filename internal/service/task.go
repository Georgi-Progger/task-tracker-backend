...
func (t *taskService) CreateTask(ctx context.Context, task entity.Task) (string, error) {
	taskId, err := t.taskRepo.CreateTask(ctx, task.Title, task.Text, task.Status, task.UserId)
	if err != nil {
...
func (t *taskService) UpdateTask(ctx context.Context, task entity.Task) error {
