...
func (t *taskService) CountUserTask(ctx context.Context) ([]model.TaskCounter, error) {
	taskCounters, err := t.taskRepo.CountUsersTasks(ctx)
	if err != nil {
		return nil, fmt.Errorf("could not get users tasks: %v", err)
	}
	return taskCounters, nil
}
