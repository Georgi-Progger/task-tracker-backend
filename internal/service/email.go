...
func NewEmailService(taskService taskService, producer kafka.Producer, logger logger.Logger) *emailService {
	return &emailService{
		taskService: taskService,
		producer:    producer,
		logger:      logger,
...
func (e *emailService) SendTaskCountMessage(ctx context.Context) error {
	taskCounters, err := e.taskService.CountUserTask(ctx)
	if err != nil {
